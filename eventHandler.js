    ...
    var consumer = new kafka_native.Consumer({
        ...
        receive_callback: function(data) {
            ...
            data.messages.forEach(function(m) {
                log('recived message on topic ' + m.topic + " with a partition of "+ m.partition + " with the offset of " + m.offset + " with this payload " + m.payload);
                //parse the event object
                eventData = JSON.parse(m.payload);
                //call the event function specified by the user in events
                if (events[eventData.event]) {
                    log("Calling event " + eventData.event);
                    ...
                    log("Unrecognized event" + eventData.event);
